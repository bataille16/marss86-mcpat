...
int ReorderBufferEntry::issue() {
   ThreadContext& thread = getthread();
    ...
    core.pow_total_uops++;    // committed + misspredicted 
	...
	///bataille16 increment parameters for pow opclass
	if(opclassof(uop.opcode) == 11)
	...
	if(opclassof(uop.opcode) >=20 && opclassof(uop.opcode)<=26)  	
		core.pow_fpu_accesses++; 	
...
void ReorderBufferEntry::issueast(IssueState& state, W64 assistid, W64 ra,
    ...
    if(assistid == L_ASSIST_PAUSE) {
 	...
 	core.pow_idle_cycles = THREAD_PAUSE_CYCLES + THREAD_PAUSE_CYCLES;; 
  }
...
#ifndef DISABLE_TLB
    OooCore& core = getcore();
    core.pow_dtlb_accesses++;    // First check if its a TLB hit or miss
